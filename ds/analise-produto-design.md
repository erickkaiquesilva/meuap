...
**Escopo:** Diagnóstico dos concorrentes locais + inspirações de mercado, e recomendação de solução (Design System + protótipo de home).

---

## 1. Entendimento do problema

**Problema central:** os sites de imobiliária da região (Granado Imóveis e Pedro Granado) não comunicam com clareza nem transmitem confiança suficiente para converter visitantes em leads qualificados (contato/proposta).
...
## 2. Diagnóstico (concorrentes analisados via código-fonte)

### Granado Imóveis (plataforma Kenlo)
...
- Telefone grafado com espaços entre dígitos ("(4 4) 9 9 1 2 5.0036") — tática antispam antiga que quebra o "toque para ligar" em mobile.
...
Comparando com QuintoAndar, Loft e Airbnb (ver imagens em `img-reference/`), os três padrões que mais se repetem — e que os concorrentes locais não têm — são:
...
3. **Prova social e confiança**: avaliações, selos ("Preferido dos hóspedes"), depoimentos — algo ausente nos dois concorrentes.
...
**Oportunidade de negócio:** ser a primeira imobiliária da região com um site no padrão de UX de proptechs nacionais. Isso eleva a percepção de preço/profissionalismo (importante para imóveis de médio/alto padrão) e reduz a fricção entre "pesquisar" e "chamar no WhatsApp".
...
### Direção de design
Combinar a objetividade de busca do QuintoAndar, a riqueza informativa dos cards da Loft e a hierarquia fotográfica do Airbnb, com uma camada de sofisticação visual própria (tipografia serifada nos títulos + paleta Blueberry/Cream) que diferencia o site dos concorrentes locais — hoje nenhum deles usa serifa ou uma paleta com essa personalidade.
...
**Existentes reaproveitáveis:** nenhum — é um projeto novo, então o DS entregue já nasce como a base única de verdade (positivo: elimina o problema #1 dos concorrentes, que é ausência total de tokens).
...
**Impacto no Design System:** positivo — centraliza tokens desde o dia 1, o que os dois concorrentes nunca tiveram. Isso barateia manutenção futura e permite expandir para novas cidades/bairros sem reescrever estilos.
...
- **Fase 3:** mapa interativo com preços nos pins (padrão QuintoAndar/Airbnb), favoritos e área logada — funcionalidades que exigem back-end e ficam fora do escopo deste protótipo estático.
...
- **Teste A/B em produção (quando o site estiver com backend):** busca com 4 campos (Cidade, Bairro, Valor, Quartos) vs. busca simplificada de 2 campos (como o Granado hoje) — para confirmar que mais filtros objetivos aumentam taxa de contato em vez de gerar abandono (hipótese baseada no padrão QuintoAndar, mas precisa ser validada com o público local).
- **Métricas de sucesso pós-lançamento:** taxa de contato (WhatsApp/formulário) por sessão, tempo médio na home, taxa de rejeição comparada aos concorrentes, e SUS (System Usability Scale) pré/pós lançamento.

---

## 6. Referências analisadas

- **Concorrentes (código-fonte):** granadoimoveis.com.br, pedrogranado.com.br
- **Inspirações (capturas de tela):** quintoandar.com.br (home, listagem, detalhe), loft.com.br (listagem, detalhe), airbnb.com.br (listagem, detalhe), referência adicional `layout-figma.png` (paleta indigo/amarelo) e `galery-example.png` (grid de galeria de fotos)
